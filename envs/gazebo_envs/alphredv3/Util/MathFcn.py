...
def auto_eval_spline(coef, order, t):
    ...
    """
    val = 0
    for idx in reversed(list(range(order+1))):
...
def rotm2quat(C):
    ...
    q1 = old_div((C[1,2] - C[2,1]), (4.0*q4))
    q2 = old_div((C[2,0] - C[0,2]), (4.0*q4))
    q3 = old_div((C[0,1] - C[1,0]), (4.0*q4))
...
def rotmRPY(r, p, y):
    R = np.zeros((3,3))
    R[0,0] = np.cos(y)*np.cos(p)
    R[0,1] = np.cos(y)*np.sin(p)*np.sin(r) - np.sin(y)*np.cos(r)
    R[0,2] = np.cos(y)*np.sin(p)*np.cos(r) + np.sin(y)*np.sin(r)

    R[1,0] = np.sin(y)*np.cos(p)
    R[1,1] = np.sin(y)*np.sin(p)*np.sin(r) + np.cos(y)*np.cos(r)
    R[1,2] = np.sin(y)*np.sin(p)*np.cos(r) - np.cos(y)*np.sin(r)

    R[2,0] = -np.sin(p)
    R[2,1] = np.cos(p)*np.sin(r)
    R[2,2] = np.cos(p)*np.cos(r)
    return R
